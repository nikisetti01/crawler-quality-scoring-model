...
ENCODER = AutoModel.from_pretrained(QUALT5_MODEL).encoder
...
class MultiModalWebClassifier(nn.Module):
    ...
    def forward(self, input_ids, attention_mask,
        ...
        text_tokens = enc_out.last_hidden_state
        ...
        uni_out = self.uni_attn(meta_tokens, text_tokens)
        meta_vec = uni_out.mean(dim=1)

        combined = torch.cat([cls, meta_vec], dim=-1)
